...
const Footer: React.FC<FooterProps> = ({
  ...
  return (
    <footer className="bg-[#0D1717] text-[#FCF9F5] relative flex flex-col w-full h-full justify-between lg:h-screen select-none">
      {/* Flickering Grid Background with Gradient Mask */}
      <div className="absolute inset-0 z-0">
        <FlickeringGrid
          ...
          flickerChance={0.08}
        />
        {/* Gradient mask - highest opacity at bottom, fades to transparent at top */}
        <div className="absolute inset-0 bg-gradient-to-t from-[#0D1717] via-[#0D1717]/80 to-transparent"></div>
      </div>
      
      <div className="container mx-auto flex flex-col md:flex-row justify-between w-full gap-4 pb-24 pt-8 px-6 md:px-10 relative z-10">
        <div className="space-y-2">
          <ul className="flex flex-wrap gap-4">
            {leftLinks.map((link, index) => (
              <li key={index}>
                <a href={link.href} className="text-sm hover:text-[#1DC6A1] transition-colors">
                  {link.label}
                </a>
              </li>
            ))}
          </ul>
          <p className="text-sm mt-4 flex items-center gap-x-1 text-[#FCF9F5]/80">
        ...
        <div className="space-y-4">
          ...
          <div className="text-right mt-4">
            <button className="text-sm hover:underline inline-flex items-center hover:text-[#1DC6A1] transition-colors">
              Back to top
            </button>
          </div>
        </div>
