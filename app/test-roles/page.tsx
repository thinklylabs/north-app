      ...
      <div className="space-y-4">
        ...
        <Card className="p-4">
          ...
          <div className="space-y-2">
            <Button onClick={initializeRoles} className="mr-2">
              Initialize All Roles (set to '{ROLES.USER}')
            </Button>
            <p className="text-sm text-gray-600">
              This will set all users without a role to '{ROLES.USER}' by default.
            </p>
          </div>
        </Card>
        ...
        {userInfo && (
          <Card className="p-4">
            ...
            <div className="space-x-2">
              <Button 
                onClick={() => setRole(userInfo.id, ROLES.USER)}
                variant={userInfo.role === ROLES.USER ? 'default' : 'outline'}
              >
                Set as {ROLES.USER}
              </Button>
              <Button 
                onClick={() => setRole(userInfo.id, ROLES.ADMIN)}
                variant={userInfo.role === ROLES.ADMIN ? 'default' : 'outline'}
              >
                Set as {ROLES.ADMIN}
              </Button>
            </div>
        ...
        <Card className="p-4">
          ...
          <div className="space-y-1">
            <p className="text-sm"><code className="bg-gray-100 px-1 rounded">{ROLES.USER}</code> - Regular user access</p>
            <p className="text-sm"><code className="bg-gray-100 px-1 rounded">{ROLES.ADMIN}</code> - Admin access</p>
          </div>
        </Card>
